...
def gaussian_peak_density(
    window: Tuple[float, float],
    mu: str = "mu",
    sigma: str = "sigma",
    nsig: str = "nsig",
) -> Callable:
    """Gaussian signal rate density for extended unbinned fits.

    Intended for :class:`analysis_utilities.unbinned_fit.UnbinnedFit` /
    :class:`iminuit.cost.ExtendedUnbinnedNLL`. Returns
    ``f(x, mu, sigma, nsig) -> (nsig, rate_density)`` where
    ``rate_density`` has units of events-per-unit-x and integrates to
    ``nsig`` over ``window``.

    Parameters
    ----------
    window : tuple(float, float)
        Fit window ``(x_lo, x_hi)`` over which the density is normalized.
    mu, sigma, nsig : str
        Parameter names on the returned callable.

    Returns
    -------
    callable
    """
    x_lo, x_hi = float(window[0]), float(window[1])

    def core(x: np.ndarray, mu_val: float, sigma_val: float,
             nsig_val: float) -> Tuple[float, np.ndarray]:
        sqrt2_sigma = np.sqrt(2.0) * sigma_val
        normalization = 0.5 * (erf((x_hi - mu_val) / sqrt2_sigma)
                                - erf((x_lo - mu_val) / sqrt2_sigma))
        x_arr = np.asarray(x, dtype=np.float64)
        z = (x_arr - mu_val) / sigma_val
        pdf = (np.exp(-0.5 * z * z)
               / (np.sqrt(2.0 * np.pi) * sigma_val * normalization))
        return nsig_val, nsig_val * pdf

    return _with_param_names(core, mu, sigma, nsig)


def exponential_bkg_density(
    window: Tuple[float, float],
    tau: str = "tau",
    nbkg: str = "nbkg",
) -> Callable:
    """Exponential background rate density for extended unbinned fits.

    Shape :math:`\\propto e^{-x/\\tau}` normalized over ``window``, scaled
    by ``nbkg`` so the integrated rate over the window equals ``nbkg``.
    ...
    -------
    callable
        ``f(x, tau, nbkg) -> (nbkg, rate_density)``.
    """
    x_lo, x_hi = float(window[0]), float(window[1])

    def core(x: np.ndarray, tau_val: float,
             nbkg_val: float) -> Tuple[float, np.ndarray]:
        shifted_hi = x_hi - x_lo
        normalization = tau_val * (1.0 - np.exp(-shifted_hi / tau_val))
        x_arr = np.asarray(x, dtype=np.float64)
        pdf = np.exp(-(x_arr - x_lo) / tau_val) / normalization
        return nbkg_val, nbkg_val * pdf

    return _with_param_names(core, tau, nbkg)


def low_exp_tail_density(
    window: Tuple[float, float],
    mu: str = "mu",
    sigma: str = "sigma",
    nsig: str = "n_low_exp",
    ...
    """Low-energy exponential tail, density form for extended unbinned fits.

    Same shape as :func:`low_exp_tail` (binned):

    .. math::

       \\mathrm{shape}(x) = e^{(x-\\mu)/\\tau}
                          \\operatorname{erfc}\\!\\left(\\frac{x-\\mu}{\\sigma\\sqrt 2}\\right)

    Normalized over ``window`` and scaled so the rate-density integrates
    to the yield parameter.

    Returns
    -------
    callable
        ``f(x, mu, sigma, n_low_exp, tau_low_exp) -> (yield, rate_density)``.
    """
    x_lo, x_hi = float(window[0]), float(window[1])

    def core(x: np.ndarray, mu_val: float, sigma_val: float,
             n_val: float, tau_val: float) -> Tuple[float, np.ndarray]:
        x_arr = np.asarray(x, dtype=np.float64)
        y = x_arr - mu_val
        sqrt2_sigma = np.sqrt(2.0) * sigma_val
        shape = np.exp(y / tau_val) * erfc(y / sqrt2_sigma)
        bounds = _exp_tail_antideriv(
            np.array([x_lo - mu_val, x_hi - mu_val], dtype=np.float64),
            sigma_val, tau_val)
        norm = float(bounds[1] - bounds[0])
        if not np.isfinite(norm) or norm <= 0.0:
            return n_val, np.zeros_like(x_arr)
        return n_val, n_val * shape / norm

    return _with_param_names(core, mu, sigma, nsig, decay)


def high_exp_tail_density(
    window: Tuple[float, float],
    mu: str = "mu",
    sigma: str = "sigma",
    nsig: str = "n_high_exp",
    ...
    """High-energy exponential tail, density form for extended unbinned fits.

    Mirror of :func:`low_exp_tail_density` about ``mu``:

    .. math::

       \\mathrm{shape}(x) = e^{(\\mu-x)/\\tau}
                          \\operatorname{erfc}\\!\\left(\\frac{\\mu-x}{\\sigma\\sqrt 2}\\right)

    Returns
    -------
    callable
        ``f(x, mu, sigma, n_high_exp, tau_high_exp) -> (yield, rate_density)``.
    """
    x_lo, x_hi = float(window[0]), float(window[1])
    ...
    def core(x: np.ndarray, mu_val: float, sigma_val: float,
        ...
        z = mu_val - x_arr
        sqrt2_sigma = np.sqrt(2.0) * sigma_val
        shape = np.exp(z / tau_val) * erfc(z / sqrt2_sigma)
        bounds = _exp_tail_antideriv(
            np.array([mu_val - x_hi, mu_val - x_lo], dtype=np.float64),
            sigma_val, tau_val)
        norm = float(bounds[1] - bounds[0])
        if not np.isfinite(norm) or norm <= 0.0:
            return n_val, np.zeros_like(x_arr)
...
def low_lin_tail_density(
    ...
    """Low-energy linear tail, density form for extended unbinned fits.

    Same shape as :func:`low_lin_tail` (binned):

    .. math::

       \\mathrm{shape}(x) = \\left[1 + s (x-\\mu)\\right]
                          \\operatorname{erfc}\\!\\left(\\frac{x-\\mu}{\\sigma\\sqrt 2}\\right)

    Normalized over ``window``. Constrain ``s`` such that the shape is
    non-negative across the window (``1 + s y ≥ 0`` for ``y`` in the
    window's offset range).

    Returns
    -------
    callable
        ``f(x, mu, sigma, n_low_lin, s_low_lin) -> (yield, rate_density)``.
    """
    x_lo, x_hi = float(window[0]), float(window[1])

    def core(x: np.ndarray, mu_val: float, sigma_val: float,
             n_val: float, slope_val: float) -> Tuple[float, np.ndarray]:
        x_arr = np.asarray(x, dtype=np.float64)
        y = x_arr - mu_val
        sqrt2_sigma = np.sqrt(2.0) * sigma_val
        shape = (1.0 + slope_val * y) * erfc(y / sqrt2_sigma)
        bounds = _low_lin_antideriv(
            np.array([x_lo - mu_val, x_hi - mu_val], dtype=np.float64),
            sigma_val, slope_val)
        norm = float(bounds[1] - bounds[0])
        if not np.isfinite(norm) or norm <= 0.0:
    ...
    return _with_param_names(core, mu, sigma, nsig, slope)


def chebyshev1_bkg_density(
    window: Tuple[float, float],
    c1: str = "c1",
    nbkg: str = "nbkg",
) -> Callable:
    """Chebyshev-order-1 background rate density for extended unbinned fits.

    Density :math:`\\propto 1 + c_1 \\, T_1(u)` where
    :math:`u = 2 (x - x_\\mathrm{mid}) / (x_\\mathrm{hi} - x_\\mathrm{lo})`
    maps the fit window to ``[-1, 1]`` and :math:`T_1(u) = u`. Mathematically
    equivalent to :func:`linear_bkg_density` but parameterized so the
    coefficient ``c1`` is naturally bounded on ``[-1, 1]`` — the positivity
    limit of the density at the window edges. Set limits ``(-0.99, 0.99)``
    or similar to stay strictly positive.
    ...
    def core(x: np.ndarray, c1_val: float,
             nbkg_val: float) -> Tuple[float, np.ndarray]:
        x_arr = np.asarray(x, dtype=np.float64)
        u = (x_arr - x_mid) / half_width
        pdf = (1.0 + c1_val * u) / width
        return nbkg_val, nbkg_val * pdf

    return _with_param_names(core, c1, nbkg)


def linear_bkg_density(
    window: Tuple[float, float],
    slope: str = "slope",
    nbkg: str = "nbkg",
) -> Callable:
    """Linear background rate density for extended unbinned fits.

    Density :math:`\\propto 1 + s \\cdot (x - x_\\mathrm{mid})` normalized
    over ``window`` (midpoint parametrization — slope leaves the integral
    unchanged), scaled by ``nbkg``. Handles either slope direction; user
    should constrain ``slope`` so the density stays positive across the
    window (``|slope| < 2 / (x_hi - x_lo)``).
    ...
    -------
    callable
        ``f(x, slope, nbkg) -> (nbkg, rate_density)``.
    """
    x_lo, x_hi = float(window[0]), float(window[1])
    x_mid = 0.5 * (x_lo + x_hi)
    width = x_hi - x_lo

    def core(x: np.ndarray, slope_val: float,
             nbkg_val: float) -> Tuple[float, np.ndarray]:
        x_arr = np.asarray(x, dtype=np.float64)
        pdf = (1.0 + slope_val * (x_arr - x_mid)) / width
        return nbkg_val, nbkg_val * pdf

    return _with_param_names(core, slope, nbkg)


def compose_densities(**components: Callable) -> Callable:
    """Sum component rate-density models for the unbinned-fit convention.

    Each component callable must have signature
    ``sub(x, *params) -> (yield_i, rate_density_i)`` where
    ``rate_density_i`` integrates to ``yield_i`` over the common fit
    window. The composed callable returns
    ``(sum(yield_i), sum(rate_density_i))`` — the form
    :class:`iminuit.cost.ExtendedUnbinnedNLL` expects.

    Parameters
    ----------
    **components
        Keyword-named component callables.

    Returns
    -------
    callable
        A model callable with ``.components`` metadata attached (the input
        dict), suitable for passing to
        :meth:`analysis_utilities.unbinned_fit.UnbinnedFit.set_events`.
    """
    if not components:
        raise ValueError("compose_densities() requires at least one component")
    ...
    def core(x: np.ndarray,
             *values: float) -> Tuple[float, np.ndarray]:
        arg_map = {name: val for name, val in zip(all_params, values)}
        total_yield: float = 0.0
        total_rate: np.ndarray | None = None
        for comp_name, comp in components.items():
            comp_args = [arg_map[p] for p in component_params[comp_name]]
            y_i, rate_i = comp(x, *comp_args)
            total_yield += float(y_i)
            rate_i_arr = np.asarray(rate_i, dtype=np.float64)
            total_rate = (rate_i_arr
                          if total_rate is None else total_rate + rate_i_arr)
        return total_yield, total_rate

...
Double_t FittingFunctions::DoublePeakFunction(Double_t *x, Double_t *par) {
  // Enforce: higher mu peak must have higher sigma
  if ((par[10] > par[0] && par[11] < par[1]) ||
      (par[0] > par[10] && par[1] < par[11]))
    return 0;
...
Double_t FittingFunctions::TriplePeakFunction(Double_t *x, Double_t *par) {
  for (Int_t a = 0; a < 3; a++) {
    for (Int_t b = a + 1; b < 3; b++) {
      Double_t mu_a = par[a * 10], mu_b = par[b * 10];
      Double_t sig_a = par[a * 10 + 1], sig_b = par[b * 10 + 1];
      if ((mu_b > mu_a && sig_b < sig_a) || (mu_a > mu_b && sig_a < sig_b))
        return 0;
    }
  }
...
Double_t FittingFunctions::EMGLowComponent(Double_t x, Double_t mu,
                                           Double_t sigma, Double_t w,
                                           Double_t tau) {
  if (tau <= 0 || sigma <= 0 || w <= 0)
    return 0;

  Double_t y = x - mu;
  Double_t s_over_t = sigma / tau;
  Double_t exp_arg = y / tau + 0.5 * s_over_t * s_over_t;

  if (exp_arg > 500.0)
    return 0;

  Double_t erfc_arg = (s_over_t + y / sigma) / TMath::Sqrt(2.0);

  if (erfc_arg > 10.0)
    return 0;

  return (w / (2.0 * tau)) * TMath::Exp(exp_arg) * TMath::Erfc(erfc_arg);
}

Double_t FittingFunctions::EMGHighComponent(Double_t x, Double_t mu,
                                            Double_t sigma, Double_t w,
                                            Double_t tau) {
  if (tau <= 0 || sigma <= 0 || w <= 0)
    return 0;
...
Double_t FittingFunctions::HyperEMGPeak(Double_t *x, Double_t *par) {
  Double_t mu = par[0];
  Double_t sigma = par[1];
  Double_t amplitude = par[2];
  Double_t w_low1 = par[3];
  Double_t tau_low1 = par[4];
  Double_t w_low2 = par[5];
  Double_t tau_low2 = par[6];
  Double_t w_high1 = par[7];
  Double_t tau_high1 = par[8];
  Double_t w_high2 = par[9];
  Double_t tau_high2 = par[10];

  Double_t w_total = w_low1 + w_low2 + w_high1 + w_high2;
  if (w_total <= 0)
    return par[12] * x[0] + par[11];

  Double_t nw_low1 = w_low1 / w_total;
  Double_t nw_low2 = w_low2 / w_total;
  Double_t nw_high1 = w_high1 / w_total;
  Double_t nw_high2 = w_high2 / w_total;

  Double_t xv = x[0];

  Double_t peak_val = EMGLowComponent(xv, mu, sigma, nw_low1, tau_low1) +
                      EMGLowComponent(xv, mu, sigma, nw_low2, tau_low2) +
                      EMGHighComponent(xv, mu, sigma, nw_high1, tau_high1) +
                      EMGHighComponent(xv, mu, sigma, nw_high2, tau_high2);

  Double_t bkg_constant = par[11];
  Double_t lin_bkg_slope = par[12];

  return amplitude * peak_val + lin_bkg_slope * xv + bkg_constant;
}

Double_t FittingFunctions::DoublePeakHyperEMG(Double_t *x, Double_t *par) {
  Double_t mu1 = par[0], mu2 = par[kHyperEMGParamsPerPeak];
  Double_t sig1 = par[1], sig2 = par[kHyperEMGParamsPerPeak + 1];
  if ((mu2 > mu1 && sig2 < sig1) || (mu1 > mu2 && sig1 < sig2))
    return 0;
  ...
  for (Int_t p = 0; p < 2; p++) {
    Int_t off = p * kHyperEMGParamsPerPeak;
    Double_t mu = par[off + 0];
    Double_t sigma = par[off + 1];
    Double_t amplitude = par[off + 2];
    Double_t w_low1 = par[off + 3];
    Double_t tau_low1 = par[off + 4];
    Double_t w_low2 = par[off + 5];
    Double_t tau_low2 = par[off + 6];
    Double_t w_high1 = par[off + 7];
    Double_t tau_high1 = par[off + 8];
    Double_t w_high2 = par[off + 9];
    Double_t tau_high2 = par[off + 10];
    ...
    Double_t peak_val = EMGLowComponent(xv, mu, sigma, nw_low1, tau_low1) +
                        ...
                        EMGHighComponent(xv, mu, sigma, nw_high1, tau_high1) +
                        EMGHighComponent(xv, mu, sigma, nw_high2, tau_high2);
    result += amplitude * peak_val;
  }

  Int_t bkg_off = 2 * kHyperEMGParamsPerPeak;
  result += par[bkg_off + 1] * xv + par[bkg_off];
  return result;
}

Double_t FittingFunctions::TriplePeakHyperEMG(Double_t *x, Double_t *par) {
  Int_t ppp = kHyperEMGParamsPerPeak;
  for (Int_t a = 0; a < 3; a++) {
    for (Int_t b = a + 1; b < 3; b++) {
      Double_t mu_a = par[a * ppp], mu_b = par[b * ppp];
      Double_t sig_a = par[a * ppp + 1], sig_b = par[b * ppp + 1];
      if ((mu_b > mu_a && sig_b < sig_a) || (mu_a > mu_b && sig_a < sig_b))
        return 0;
    }
  }
  ...
  Int_t bkg_off = 3 * kHyperEMGParamsPerPeak;
  result += par[bkg_off + 1] * xv + par[bkg_off];
  return result;
}

void FittingFunctions::SetHyperEMGParNames(TF1 *func, Int_t peak_offset,
                                           const TString &peak_label) {
  TString prefix = peak_label.Length() > 0 ? peak_label + "_" : "";
  func->SetParName(peak_offset + 0, prefix + "Mu");
  func->SetParName(peak_offset + 1, prefix + "Sigma");
  func->SetParName(peak_offset + 2, prefix + "Amplitude");
  func->SetParName(peak_offset + 3, prefix + "WLow1");
  func->SetParName(peak_offset + 4, prefix + "TauLow1");
  func->SetParName(peak_offset + 5, prefix + "WLow2");
  func->SetParName(peak_offset + 6, prefix + "TauLow2");
  func->SetParName(peak_offset + 7, prefix + "WHigh1");
  func->SetParName(peak_offset + 8, prefix + "TauHigh1");
  func->SetParName(peak_offset + 9, prefix + "WHigh2");
  func->SetParName(peak_offset + 10, prefix + "TauHigh2");
}

void FittingFunctions::SetHyperEMGParLimits(TF1 *func, Int_t peak_offset) {
...
void FittingFunctions::SetHyperEMGInitialParams(TF1 *func, Int_t peak_offset,
                                                ...
                                                Double_t sigma_init,
                                                Double_t amp_init) {
  func->SetParameter(peak_offset + 0, mu_init);
  func->SetParameter(peak_offset + 1, sigma_init);
  func->SetParameter(peak_offset + 2, amp_init);
  func->SetParameter(peak_offset + 3, 0.5);
  func->SetParameter(peak_offset + 4, 0.8);
  func->SetParameter(peak_offset + 5, 0.2);
  func->SetParameter(peak_offset + 6, 5.0);
  func->SetParameter(peak_offset + 7, 0.3);
  func->SetParameter(peak_offset + 8, 1.0);
  func->SetParameter(peak_offset + 9, 0.0);
  func->SetParameter(peak_offset + 10, 3.0);
  func->FixParameter(peak_offset + 9, 0.0);
  func->FixParameter(peak_offset + 10, 3.0);
}

void FittingFunctions::SaveHyperEMGParams(TF1 *func, Double_t rlo,
                                          Double_t rhi, const TString &input,
                                          const TString &peak) {
  gSystem->mkdir("plots/fits", kTRUE);
  TString fn = "plots/fits/" + peak + "_" + input + ".hemg";
  std::ofstream out(fn.Data());
  if (!out.is_open()) {
    std::cerr << "WARNING: Could not save hyper-EMG params to " << fn
              << std::endl;
    return;
  }
  out << std::setprecision(15);
  ...
  for (Int_t i = 0; i < func->GetNpar(); i++) {
    ...
    Bool_t fixed = (lo >= hi);
    out << func->GetParName(i) << " " << func->GetParameter(i) << " "
        << (fixed ? 1 : 0) << "\n";
  }
  out.close();
  std::cout << "Saved hyper-EMG params to " << fn << std::endl;
}

Bool_t FittingFunctions::LoadHyperEMGParams(TF1 *func, Double_t &rlo,
                                            Double_t &rhi,
                                            const TString &input,
                                            const TString &peak) {
  TString fn = "plots/fits/" + peak + "_" + input + ".hemg";
  ...
  if (token == "RANGE") {
    in >> rlo >> rhi;
    func->SetRange(rlo, rhi);
  }

  Double_t val;
  Int_t fixed;
  Int_t idx = 0;
  while (in >> token >> val >> fixed && idx < func->GetNpar()) {
    func->SetParameter(idx, val);
    if (fixed)
      func->FixParameter(idx, val);
    idx++;
  }
  in.close();

  if (idx != func->GetNpar()) {
    std::cerr << "WARNING: Parameter count mismatch in " << fn << " (expected "
              << func->GetNpar() << ", got " << idx << ")" << std::endl;
    return kFALSE;
  }

  std::cout << "Loaded hyper-EMG params from " << fn << std::endl;
  return kTRUE;
}

HyperEMGPeakFitResult FittingFunctions::ExtractHyperEMGPeak(TF1 *func,
                                                            Int_t offset) {
  HyperEMGPeakFitResult peak;
  peak.mu = func->GetParameter(offset + 0);
  peak.mu_error = func->GetParError(offset + 0);
  peak.sigma = func->GetParameter(offset + 1);
  peak.sigma_error = func->GetParError(offset + 1);
  peak.amplitude = func->GetParameter(offset + 2);
  peak.amplitude_error = func->GetParError(offset + 2);
  peak.w_low1 = func->GetParameter(offset + 3);
  peak.w_low1_error = func->GetParError(offset + 3);
  peak.tau_low1 = func->GetParameter(offset + 4);
  peak.tau_low1_error = func->GetParError(offset + 4);
  peak.w_low2 = func->GetParameter(offset + 5);
  peak.w_low2_error = func->GetParError(offset + 5);
  peak.tau_low2 = func->GetParameter(offset + 6);
  peak.tau_low2_error = func->GetParError(offset + 6);
  peak.w_high1 = func->GetParameter(offset + 7);
  peak.w_high1_error = func->GetParError(offset + 7);
  peak.tau_high1 = func->GetParameter(offset + 8);
  peak.tau_high1_error = func->GetParError(offset + 8);
  peak.w_high2 = func->GetParameter(offset + 9);
  peak.w_high2_error = func->GetParError(offset + 9);
  peak.tau_high2 = func->GetParameter(offset + 10);
  peak.tau_high2_error = func->GetParError(offset + 10);
  return peak;
}

void FittingFunctions::SetupHyperEMGBackground(TF1 *func, Int_t bkg_offset,
                                               Double_t peak_height,
                                               Bool_t use_flat_bkg) {
  func->SetParName(bkg_offset, "BkgConstant");
  func->SetParName(bkg_offset + 1, "LinBkgSlope");
  func->SetParameter(bkg_offset, 0);
  func->SetParLimits(bkg_offset, 0, peak_height);
  if (use_flat_bkg) {
    func->FixParameter(bkg_offset + 1, 0);
  } else {
    func->SetParameter(bkg_offset + 1, 0);
    func->SetParLimits(bkg_offset + 1, -1000, 1000);
  }
}

void FittingFunctions::PlotHyperEMGFit(TH1 *hist, TF1 *func,
  ...
  Double_t min_disp = 0.9 * range_low;
  Double_t max_disp = 1.1 * range_high;

  hist->GetXaxis()->SetRangeUser(min_disp, max_disp);
  hist->GetXaxis()->SetLabelSize(0);
  hist->GetXaxis()->SetTitleSize(0);
  hist->SetLineColor(kViolet);
  hist->GetYaxis()->SetTitleOffset(1);
  hist->SetLineWidth(PlottingUtils::GetLineWidth());
  hist->Draw();
  ...
  Int_t ppp = kHyperEMGParamsPerPeak;
  Int_t bkg_off = num_peaks * ppp;
  TF1 *background = new TF1("hemg_bkg", LinearBackground, range_low,
                             range_high, 2);
  background->SetParameter(0, func->GetParameter(bkg_off));
  background->SetParameter(1, func->GetParameter(bkg_off + 1));
  background->SetLineColor(kGreen);
  background->SetNpx(1000);
  background->Draw("same");

  Int_t npts = 1000;
  Double_t x_step = (range_high - range_low) / (npts - 1);

  Int_t line_styles[3] = {1, 3, 4};

  for (Int_t p = 0; p < num_peaks; p++) {
    Int_t off = p * ppp;
    Int_t lstyle = line_styles[p < 3 ? p : 2];

    Double_t mu = func->GetParameter(off + 0);
    Double_t sigma = func->GetParameter(off + 1);
    Double_t amp = func->GetParameter(off + 2);
    Double_t w_low1 = func->GetParameter(off + 3);
    Double_t tau_low1 = func->GetParameter(off + 4);
    Double_t w_low2 = func->GetParameter(off + 5);
    Double_t tau_low2 = func->GetParameter(off + 6);
    Double_t w_high1 = func->GetParameter(off + 7);
    Double_t tau_high1 = func->GetParameter(off + 8);
    Double_t w_high2 = func->GetParameter(off + 9);
    Double_t tau_high2 = func->GetParameter(off + 10);
    ...
    for (Int_t i = 0; i < npts; i++) {
      Double_t x = range_low + i * x_step;
      Double_t val =
          amp * (EMGLowComponent(x, mu, sigma, nw_low1, tau_low1) +
                 EMGLowComponent(x, mu, sigma, nw_low2, tau_low2) +
                 EMGHighComponent(x, mu, sigma, nw_high1, tau_high1) +
                 EMGHighComponent(x, mu, sigma, nw_high2, tau_high2));
      peak_graph->SetPoint(i, x, val + background->Eval(x));
    }
    peak_graph->SetLineColor(kBlack);
    peak_graph->SetLineStyle(lstyle);
    peak_graph->SetLineWidth(PlottingUtils::GetLineWidth());
    peak_graph->Draw("L same");

    TGraph *low1_graph = new TGraph(npts);
    for (Int_t i = 0; i < npts; i++) {
      Double_t x = range_low + i * x_step;
      Double_t val = amp * EMGLowComponent(x, mu, sigma, nw_low1, tau_low1);
      low1_graph->SetPoint(i, x, val + background->Eval(x));
    }
    low1_graph->SetLineColor(kRed);
    low1_graph->SetLineStyle(lstyle);
    low1_graph->SetLineWidth(PlottingUtils::GetLineWidth());
    low1_graph->Draw("L same");

    TGraph *low2_graph = new TGraph(npts);
    for (Int_t i = 0; i < npts; i++) {
      Double_t x = range_low + i * x_step;
      Double_t val = amp * EMGLowComponent(x, mu, sigma, nw_low2, tau_low2);
      low2_graph->SetPoint(i, x, val + background->Eval(x));
    }
    low2_graph->SetLineColor(kGray);
    low2_graph->SetLineStyle(lstyle);
    low2_graph->SetLineWidth(PlottingUtils::GetLineWidth());
    low2_graph->Draw("L same");

    TGraph *high1_graph = new TGraph(npts);
    for (Int_t i = 0; i < npts; i++) {
      Double_t x = range_low + i * x_step;
      Double_t val = amp * EMGHighComponent(x, mu, sigma, nw_high1, tau_high1);
      high1_graph->SetPoint(i, x, val + background->Eval(x));
    }
    high1_graph->SetLineColor(kOrange);
    high1_graph->SetLineStyle(lstyle);
    high1_graph->SetLineWidth(PlottingUtils::GetLineWidth());
    high1_graph->Draw("L same");

    TGraph *high2_graph = new TGraph(npts);
    for (Int_t i = 0; i < npts; i++) {
      Double_t x = range_low + i * x_step;
      Double_t val = amp * EMGHighComponent(x, mu, sigma, nw_high2, tau_high2);
      high2_graph->SetPoint(i, x, val + background->Eval(x));
    }
    high2_graph->SetLineColor(kMagenta);
    high2_graph->SetLineStyle(lstyle);
    high2_graph->SetLineWidth(PlottingUtils::GetLineWidth());
    high2_graph->Draw("L same");
  }
...
HyperEMGFitResult FittingFunctions::FitSingleHyperEMG(
    TH1 *hist, Double_t fit_low, Double_t fit_high, Bool_t use_flat_bkg,
    const TString &label, const TString &peak_name, Bool_t interactive) {
  HyperEMGFitResult result;
  result.peaks.emplace_back();
  if (!hist)
    return result;

  Int_t npar = kHyperEMGParamsPerPeak + 2;
  TF1 *func = new TF1("hemg_" + peak_name + "_" + label, &HyperEMGPeak,
                       fit_low, fit_high, npar);

  SetHyperEMGParNames(func, 0);
  SetHyperEMGParLimits(func, 0);
  func->SetParLimits(0, fit_low, fit_high);

  Int_t max_bin = hist->GetMaximumBin();
  Double_t peak_height = hist->GetBinContent(max_bin);
  Double_t mu_init = hist->GetBinCenter(max_bin);
  SetHyperEMGInitialParams(func, 0, mu_init, 1.0, peak_height);

  SetupHyperEMGBackground(func, kHyperEMGParamsPerPeak, peak_height,
                          use_flat_bkg);
  ...
  if (interactive) {
    if (LoadHyperEMGParams(func, rlo, rhi, label, peak_name)) {
      ...
      if (LaunchInteractiveFitEditor(hist, func, rlo, rhi, 1,
                                     peak_name + " / " + label)) {
        func->GetRange(rlo, rhi);
        chi2 = func->GetChisquare() / func->GetNDF();
        SaveHyperEMGParams(func, rlo, rhi, label, peak_name);
        fit_valid = kTRUE;
  ...
  if (fit_valid) {
    TString chi2label = Form("#chi^{2}/ndf = %.3f", chi2);
    PlotHyperEMGFit(hist, func, rlo, rhi, 1, label, peak_name, chi2label);

    result.peaks[0] = ExtractHyperEMGPeak(func, 0);
    result.bkg_constant = func->GetParameter(kHyperEMGParamsPerPeak);
    result.bkg_constant_error = func->GetParError(kHyperEMGParamsPerPeak);
    result.lin_bkg_slope = func->GetParameter(kHyperEMGParamsPerPeak + 1);
    result.lin_bkg_slope_error = func->GetParError(kHyperEMGParamsPerPeak + 1);
...
HyperEMGFitResult FittingFunctions::FitDoubleHyperEMG(
    TH1 *hist, Double_t fit_low, Double_t fit_high, Double_t mu1_init,
    Double_t mu2_init, Bool_t use_flat_bkg, const TString &label,
  ...
  Int_t ppp = kHyperEMGParamsPerPeak;
  Int_t npar = 2 * ppp + 2;
  TF1 *func = new TF1("hemg_" + peak_name + "_" + label, &DoublePeakHyperEMG,
                       fit_low, fit_high, npar);
  ...
  for (Int_t p = 0; p < 2; p++) {
    Int_t off = p * ppp;
    TString prefix = Form("P%d_", p + 1);
    SetHyperEMGParNames(func, off, prefix);
    SetHyperEMGParLimits(func, off);
    func->SetParLimits(off, fit_low, fit_high);
  }

  SetHyperEMGInitialParams(func, 0, mu1_init, 1.0, peak_height * 0.6);
  SetHyperEMGInitialParams(func, ppp, mu2_init, 1.0, peak_height * 0.4);

  Int_t bkg_off = 2 * ppp;
  SetupHyperEMGBackground(func, bkg_off, peak_height, use_flat_bkg);
  ...
  if (fit_valid) {
    TString chi2label = Form("#chi^{2}/ndf = %.3f", chi2);
    PlotHyperEMGFit(hist, func, rlo, rhi, 2, label, peak_name, chi2label);
...
HyperEMGFitResult FittingFunctions::FitTripleHyperEMG(
    TH1 *hist, Double_t fit_low, Double_t fit_high, Double_t mu1_init,
    Double_t mu2_init, Double_t mu3_init, Bool_t use_flat_bkg,
    const TString &label, const TString &peak_name, Bool_t interactive) {
  HyperEMGFitResult result;
  result.peaks.emplace_back();
  result.peaks.emplace_back();
  ...
  Int_t ppp = kHyperEMGParamsPerPeak;
  Int_t npar = 3 * ppp + 2;
  TF1 *func = new TF1("hemg_" + peak_name + "_" + label, &TriplePeakHyperEMG,
                       fit_low, fit_high, npar);
  ...
  SetHyperEMGInitialParams(func, 0, mu1_init, 1.0, peak_height * 0.4);
  SetHyperEMGInitialParams(func, ppp, mu2_init, 1.0, peak_height * 0.3);
  SetHyperEMGInitialParams(func, 2 * ppp, mu3_init, 1.0, peak_height * 0.3);
  ...
  if (fit_valid) {
    TString chi2label = Form("#chi^{2}/ndf = %.3f", chi2);
    PlotHyperEMGFit(hist, func, rlo, rhi, 3, label, peak_name, chi2label);
